...
describe('getOpportunities', () => {
  ...
  it('filters by search matching summary', () => {
    const result = getOpportunities({ search: 'law enforcement skills' });
    expect(result.total).toBeGreaterThanOrEqual(1);
    expect(result.data.some((o) => o.slug === 'raleigh-police-explorer-post-911')).toBe(true);
  });
  ...
  it('filters by grade', () => {
    const result = getOpportunities({ grade: 7, per_page: 100 });
    expect(result.total).toBeGreaterThanOrEqual(1);
    result.data.forEach((opp) => {
    ...
    // Museum teen sketch program includes grade 7 (grades 7–12)
    expect(result.data.some((o) => o.slug === 'nc-museum-of-art-teens-sketch-the-galleries')).toBe(true);
    // RLT Teens on Stage is grades 10+ only
    expect(result.data.some((o) => o.slug === 'raleigh-little-theatre-teens-on-stage')).toBe(false);
  });

  it('filters by age', () => {
    const result = getOpportunities({ age: 12, per_page: 100 });
    expect(result.total).toBeGreaterThanOrEqual(1);
    result.data.forEach((opp) => {
    ...
    // Police Explorers includes age 12 (cadets 12–17)
    expect(result.data.some((o) => o.slug === 'raleigh-police-explorer-post-911')).toBe(true);
    // Raleigh CIT program is ages 14–16 only
    expect(result.data.some((o) => o.slug === 'raleigh-counselor-in-training-cit-program')).toBe(false);
  });
